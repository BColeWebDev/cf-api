...
const newTrainingDay = await Regiment.updateOne({_id:req.params.id},response[0])
...
const UpdateTrainingDay = async (req:Request,res:Response) =>{
    ...
    try {
        ...
        if(response?.routines?.filter((value)=>value?.day === days[Number(req.body.day)])?.length === 1){
        ...
        response!.days = response!.days.filter((value)=>value !== days[Number(req.body.day)])
         ...
         Regiment.findOne({_id:req.params.id},(err:any,results:any) =>{
            if(!results){
                res.status(404).json({message:"No Regiment Id found"})
                        ...
                        results.days = results.days.filter((value:any) =>value !== results.routines.id(req.body._id).day)
