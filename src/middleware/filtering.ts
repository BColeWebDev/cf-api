...
const Filtering = (req: any, results: any[]) => {
  try {
      ...
      let BodyPart = req.query.bodyPart !== undefined ?req.query.bodyPart.split(',') : []
      let Equipment = req.query.equipment !== undefined ? req.query.equipment.split(',') : []
      let Target = req.query.target !== undefined ? req.query.target.split(',') : []
